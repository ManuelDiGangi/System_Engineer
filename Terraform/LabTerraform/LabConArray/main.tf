...
resource "proxmox_vm_qemu" "cloned_vms" {
 ...
 provisioner "remote-exec" {
  ...
  inline = [
    "powershell -ExecutionPolicy Bypass -File C:\\script\\post-create-config.ps1 -NewHostname ${var.hostnames[${var.cont}]} -IPAddress ${var.ip_addresses[${var.cont}]} -Gateway ${var.gateway} -DNS1 ${var.dns1} -DNS2 ${var.dns2}"
